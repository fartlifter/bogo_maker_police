    ...
    # 전체 Python Streamlit 코드 — 앞서 분할했던 모든 기능 포함
    ...
    def fetch_contents(articles, content_func):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(content_func, a['url']): a for a in articles}
    ...
    def get_newsis_content(url):
        ...
        div = soup.find("div", class_="viewer")
    ...
    def get_naver_content(url):
        ...
        if html.status_code == 200:
            ...
            div = soup.find("div", id="newsct_article")
            return div.get_text(separator="\n", strip=True) if div else None
        return None

    def collect_newsis_articles(start_dt, end_dt, selected_keywords, use_filter, status):
        articles, page = [], 1
        while True:
            status.markdown(f"🔄 뉴시스 페이지 {page} 수집 중...")
            res = requests.get(f"https://www.newsis.com/realnews/?cid=realnews&day=today&page={page}", headers=headers)
            ...
            for item in items:
                ...
                articles.append({
                    ...
                    "datetime": dt,
                    "url": "https://www.newsis.com" + title_tag['href']
                })
            page += 1
            t.sleep(0.2)
        results = fetch_contents(articles, get_newsis_content)
        if use_filter:
            results = [a for a in results if any(kw in (a.get('content') or '') for kw in selected_keywords)]
        status.markdown("✅ 뉴시스 수집 완료")
        return results

    def collect_yonhap_articles(start_dt, end_dt, selected_keywords, use_filter, status):
        articles, page = [], 1
        while True:
            status.markdown(f"🔄 연합뉴스 페이지 {page} 수집 중...")
            res = requests.get(f"https://www.yna.co.kr/news/{page}?site=navi_latest_depth01", headers=headers)
            soup = BeautifulSoup(res.text, "html.parser")
            ...
            for item in items:
                ...
                articles.append({
                    ...
                    "title": title_tag.text.strip(),
                    "datetime": dt,
                    "url": f"https://www.yna.co.kr/view/{item['data-cid']}"
                })
            page += 1
            t.sleep(0.2)
        results = fetch_contents(articles, get_yonhap_content)
        if use_filter:
            results = [a for a in results if any(kw in (a.get('content') or '') for kw in selected_keywords)]
        status.markdown("✅ 연합뉴스 수집 완료")
        return results
        ...
        for start in range(1, 301, 100):
            ...
            for item in res.json().get("items", []):
                ...
                if use_filter and not matched:
                    continue
                results.append({
                    "source": "단독", "title": title, "datetime": pub_date, "content": body, "matched": matched
                })
        status.markdown("✅ 단독 기사 수집 완료")
        return results
    ...
    selected_groups = st.multiselect("키워드 그룹", list(keyword_groups), default=['시경', '종혜북'])
    selected_keywords = [kw for g in selected_groups for kw in keyword_groups[g]]
    use_filter = st.checkbox("본문 키워드 필터링", value=True)
    collect_general = st.checkbox("연합뉴스·뉴시스", value=True)
    collect_danok = st.checkbox("[단독] 기사", value=True)

    if st.button("수집 시작"):
        status = st.empty()
        general_articles = danok_articles = []
        if collect_general:
            newsis = collect_newsis_articles(start_dt, end_dt, selected_keywords, use_filter, status)
            yonhap = collect_yonhap_articles(start_dt, end_dt, selected_keywords, use_filter, status)
            general_articles = newsis + yonhap
        if collect_danok:
            danok_articles = collect_naver_articles(start_dt, end_dt, selected_keywords, use_filter, status)
        status.markdown("✅ 수집 완료")
        ...
        if general_articles:
            ...
            for a in general_articles:
                ...
                st.markdown(highlight_keywords(a['content'], selected_keywords), unsafe_allow_html=True)
